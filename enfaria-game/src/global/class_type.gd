...
static func static_get_deep_class_list() -> PoolStringArray:
    var __deep_type_map = _get_deep_type_map()
    var class_list := PoolStringArray(__deep_type_map.keys())
    class_list.append_array(static_get_engine_class_list())
    return class_list
...
static func _get_path_map(p_script_map: Dictionary) -> Dictionary:
    ...
    for a_name in p_script_map:
        __path_map[p_script_map[a_name].path] = a_name
    return __path_map
...
func _get_deep_path_map(p_deep_type_map: Dictionary) -> Dictionary:
    ...
    for a_name in p_deep_type_map:
        __deep_path_map[p_deep_type_map[a_name].path] = a_name
    return __deep_path_map


# Same as _get_script_map, but it includes all resources
static func _get_deep_type_map() -> Dictionary:
    var __script_map = _get_script_map()
    var __path_map = _get_path_map(__script_map)
    var dirs = ["res://"]
    var first = true
            ...
            while file_name:
                ...
                if not file_name.begins_with("."):
                    ...
                    # Only use files with extensions
                        ...
                        var existing_name = __path_map[a_path] if __path_map.has(a_path) else ""
...
static func _get_script() -> Script:
    return load("res://src/global/class_type.gd") as Script
...
static func _convert_name_to_variant(p_name: String, p_map: Dictionary = {}):
    var ress = _convert_name_to_res(p_name, p_map)
    if ress:
        return ress
    if ClassDB.class_exists(p_name):
        return p_name
